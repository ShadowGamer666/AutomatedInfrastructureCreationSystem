...
# Obtain parameters from Desktop User.
FILETYPE=$1
...
then
        echo "xxxxxxxxxx"
        echo "Classification Model Prediction Failure Detected !!!!!"
        echo "xxxxxxxxxx"
        exit 15
fi
...
# Discovers the user's default Cloud Provider.
...
then
        echo "xxxxxxxxxx"
        echo "Extraction Model Prediction Failure Detected !!!!!"
        echo "xxxxxxxxxx"
        exit 16
fi
...
then
        ...
        echo $EXTRACT_RESULT "[<Entity> : <Entity_Count>]"
        echo "----------"
fi
...
then
        ...
        # Executes all playbooks specified by the Extraction Model.
        for playbook in "${!EXTRACT_ARRAY[@]}"
        do
                # Playbooks will always have even indexes.
                if [ $((playbook%2)) -eq 0 ]
                then
                        # Obtains the next playbook's name.
...
then
        # Ensures cleanup operation can access relevant statefile.
        if [ $CLOUD_PROVIDER = "AWS" ]
        then
                cd $INFRA_PATH/InfrastructureLib/Terraform/Projects/aws_$TEMPLATE_NAME
        elif [ $CLOUD_PROVIDER = "GOOGLE" ]
